...
GayPornArchive, GayPornPlanet, and MachoTube are live-verified Edge adapters. The default six-provider `bear` search currently returns 120 proven results (40 from each active source) in roughly 223 ms client-observed, while nonsense controls are rejected rather than surfaced as relevant results. XVideos is currently `VANTAGE_BLOCKED` from Cloudflare and BarebackBastards currently times out from the Cloudflare vantage; neither condition is disguised as an empty successful search. Providers without an Edge parser return `ADAPTER_UNIMPLEMENTED`.
