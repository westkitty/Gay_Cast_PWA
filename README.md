...
GayPornPlanet is the first live-verified Edge adapter: `bear` and `muscle` each returned 40 query-specific results with zero URL overlap in the 2026-09-16 proof run, while a nonsense query was rejected as `QUERY_FALLBACK` with zero trusted results. XVideos is currently `VANTAGE_BLOCKED` from Cloudflare and BarebackBastards currently times out from the Cloudflare vantage; neither condition is disguised as an empty successful search. Providers without an Edge parser return `ADAPTER_UNIMPLEMENTED`.
